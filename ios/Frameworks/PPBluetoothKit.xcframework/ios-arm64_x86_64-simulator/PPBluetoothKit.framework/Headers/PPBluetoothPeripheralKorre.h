...
@interface PPBluetoothPeripheralKorre : NSObject
...
/// - Parameters:
///   - info: 单个用户信息 - 对象中的每个属性都要赋值
///   - handler:  0设置成功 1设置失败
- (void)dataSyncUserInfo:(PPKorreUserModel *)info withHandler:(void(^)(NSInteger status))handler;
...
///   - handler:  0设置成功 1设置失败
- (void)dataSelectUser:(PPKorreUserModel *)userModel withHandler:(void(^)(NSInteger status))handler;
...
///   - userModel: 单个用户信息 - userId、memberId为必传项
///   - handler:  0设置成功 1设置失败
- (void)dataDeleteUser:(PPKorreUserModel *)userModel withHandler:(void(^)(NSInteger status))handler;


/// 获取设备端用户列表
/// - Parameter handler: 返回设备端所有用户的userId
- (void)dataFetchUserList:(void(^)(NSArray <PPKorreUserModel *>* infos))handler;
...
/// 同步食物信息
/// @param info 食物信息 - 对象中的每个属性都要赋值
/// @param handler 异步回调块，在主线程执行
///             - status: 0设置成功 1设置失败
///             - errorCode: 错误码
- (void)dataSyncFoodInfo:(PPKorreFoodInfo *)info withHandler:(void(^)(NSInteger status, NSString *errorCode))handler;

/// 删除食物
/// @param info 传入食物的 foodNo
/// @param handler 异步回调块，在主线程执行
///             - status: 0-成功，1-失败
///             - errorCode: 失败码
- (void)dataDeleteFood:(PPKorreFoodInfo *)info withHandler:(void(^)(NSInteger status, NSString *errorCode))handler;

/// 获取食物编号和远程ID
/// - handler： foodNo 和 foodRemoteId 有值
- (void)dataFetchFoodIDList:(void(^)(NSArray <PPKorreFoodInfo *>* infos))handler;


/// 获取指定成员的食物历史记录
/// @param handler 异步回调块，在主线程执行
///             - history: 成功时返回 PPBluetoothScaleBaseModel 数组
///             - status: 0-成功，1-失败
- (void)dataFetchFoodHistory:(PPKorreUserModel *)userMode withHandler:(void(^)(NSArray <PPBluetoothScaleBaseModel *>* history, int status))handler;
...
///             - status: 0设置成功 1设置失败
///             - errorCode: 错误码
- (void)dataSyncNutrientsInfo:(PPKorreFoodInfo *)info withHandler:(void(^)(NSInteger status, NSString *errorCode))handler;
...
///             - history: 成功时返回 PPBluetoothScaleBaseModel 数组
///             - status: 0-成功，1-失败
- (void)dataFetchSavedFoodListWithHandler:(void(^)(NSArray <PPBluetoothScaleBaseModel *>* list, int status))handler;


/// 展示 同步食物中 状态（设备会展示"同步中"画面）
/// @param handler 异步回调块，在主线程执行
///             - status: 0-成功，1-失败
- (void)showFoodSyncStatusWithHandler:(void(^)(NSInteger status))handler;

/// 隐藏 同步食物中 状态（设备退出"同步中"画面）
/// @param handler 异步回调块，在主线程执行
///             - status: 0-成功，1-失败
- (void)hideFoodSyncStatusWithHandler:(void(^)(NSInteger status))handler;
...
///             - history: 成功时返回 PPBluetoothScaleBaseModel 数组
///             - status: 0-成功，1-失败
- (void)saveFoodWithHandler:(void(^)(int status))handler;
