  ...
  Future<void> _handle(String title) async {
    ...
    try {
      if (title == DeviceMenuType.changeUnit.value) {
        ...
        _unit = _unit == PPUnitType.UnitG
            ? PPUnitType.UnitMLWater
            : PPUnitType.UnitG;
        final ret = await PPPeripheralFish.syncUnit(_unit)
      ...
      if (title == DeviceMenuType.toZero.value) {
        _updateText('toZero');
        final ret =
            PPPeripheralFish.toZero().timeout(const Duration(seconds: 3));
